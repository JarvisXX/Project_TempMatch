function histogram=templateHistogarm(img,partNum,binNum)
%% 用于求表头的投影
% binNum：横纵投影的bin数
% partNum： 截取前多少作为表头
% histogram：输出的投影向量
...
histogram(1:binNum)=...
    (count(1:binNum)-min(count(1:binNum)))./max(count(1:binNum));
...
histogram(binNum+1:2*binNum)=...
    (count(binNum+1:2*binNum)-min(count(binNum+1:2*binNum)))./max(count(binNum+1:2*binNum));
